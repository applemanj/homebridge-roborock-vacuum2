 ...
 *
 * `pv === "B01"` is TWO wire protocols, not one. The Q7 dialect (`sc*`) carries
 * an RPC envelope on datapoint 10000:
 *
 ...
 * nonsense to Apple Home. Status on a Q10 continues to come from home data over
 * HTTPS, which is a different transport and is measured to work.
 *
 * Every code below is read from python-roborock rather than guessed:
 * `roborock/data/b01_q10/b01_q10_code_mappings.py` for the datapoint numbers and
 * enums, `roborock/devices/traits/b01/q10/vacuum.py` for the payload of each
 * command, where the docstrings mark them "Verified live against ss07
 * hardware". None of it is verified by this project — there is no Q10 here.
 */

/**
 * Q10 datapoint codes (`B01_Q10_DP`). Only the ones this file uses are listed;
 * the upstream enum is far longer.
 */
const B01_Q10_DP = {
  FAULT: 90,
  COMMON: 101,
 ...
 * sends for "return to dock"; 1 washes the mop en route and 4 collects dust en
 * route, and neither is what a Matter "go home" means.
 */
...
/**
 * Q10 `wind` (suction) codes <- the plugin's v1 fan_power codes (upstream
 * `YXFanLevel`). Unlike the Q7 table, off is a real level on this family and is
 * not degraded to quiet, and max+ is 8 rather than 5.
 * @type {Record<number, number>}
 */
...
/**
 * Segment ids out of the several shapes callers use.
 *
 * This plugin's own call sites pass a single-element array wrapping a room
 * object — `[{segments: [9, 11], repeat: 1}]` — which the plain `map(Number)`
 * path would flatten to `[NaN]` and then filter away to nothing, silently
 * refusing every room clean. Unwrap that shape explicitly.
 *
 * @param {any} params
 * @returns {number[]}
 */
function normalizeSegmentIds(params) {
  ...
  if (source && !Array.isArray(source) && Array.isArray(source.segments)) {
    source = source.segments;
  }

  if (Array.isArray(source) && source.length === 1) {
    const only = source[0];
    if (only && !Array.isArray(only) && Array.isArray(only.segments)) {
  ...
  return source
    .map((entry) => {
      if (entry && typeof entry === "object") {
        const id =
          entry.segmentId ?? entry.segment_id ?? entry.id ?? entry.roomId;
        return Number(id);
      }
      return Number(entry);
    })
 ...
 * rather than invent a payload; a guessed datapoint on a robot nobody here owns
...
function translateOutgoing(method, params) {
  switch (method) {
    ...
    case "set_custom_mode": {
      ...
      const wind = Q10_V1_FAN_POWER_TO_WIND[v1Code];
...
module.exports = {
  B01_Q10_DP,
  YX_CLEAN_TASK,
  Q10_BACK_TO_DOCK_CHARGE,
  Q10_DOCK_TASK_COLLECT_DUST,
