...
const TYPES_REVERSE = Object.fromEntries(
...
class RRMapParser {
  ...
  async parsedata(buf) {
    ...
    if (!metaData.map_index) {
      this.adapter.log.error(
        `RRMapParser: Failed to parse map data. map_index was missing`
      );
      return {};
    }
        ...
        switch (type) {
          ...
          case TYPES.GOTO_PREDICTED_PATH: {
            ...
            for (let i = 0; i < length; i = i + 4) {
              result[pathType].points.push(
                this.getPointInPath(buf, dataPosition + i)
              );
            }

            if (result[pathType].points.length >= 2) {
              const lastPoint =
                result[pathType].points[result[pathType].points.length - 1];
              const secondLastPoint =
                result[pathType].points[result[pathType].points.length - 2];
          ...
          case TYPES.VIRTUAL_WALLS: {
            ...
            for (let i = 0; i < wallCount; i++) {
              ...
              result[TYPES_REVERSE[type]].push(
                this.readUInt16LE(buf, wallDataPosition, offset1, 4)
              );
            }
            break;
          }
          ...
          case TYPES.MODE_CARPET: {
            ...
            for (let i = 0; i < zoneCount; i++) {
              ...
              result[TYPES_REVERSE[type]].push(
                this.getForbiddenZone(buf, zoneDataPosition, offset1)
              );
            }
            break;
          }
          case TYPES.OBSTACLES2:
            result[TYPES_REVERSE[type]] = this.extractObstacles(
              blockBuffer,
              offset1
            );
            break;
          case TYPES.CURRENTLY_CLEANED_BLOCKS: {
            ...
            for (let i = 0; i < blockCount; i++) {
              result[TYPES_REVERSE[type]].push(
                buf.readUInt8(OFFSETS.BLOCKS + dataPosition + i)
              );
            }
            break;
          }
          case TYPES.NONCEDATA:
            ...
            this.adapter.log.debug(
              `Block type 34 debug: ${JSON.stringify(result[TYPES_REVERSE[type]])}`
            );
            break;
        }
      } else {
        this.adapter.log.warn(
          `Unknown block type! Please report this to the developer. Block type is: ${type} and a length of ${length}`
        );
        this.adapter.log.warn(
          `Unknown block type hex data: ${TYPES_REVERSE[type]} ${blockBuffer.toString("hex")}`
        );
        this.adapter.log.warn(
          `Unknown block type buffer data: ${TYPES_REVERSE[type]} ${JSON.stringify(blockBuffer)}`
        );
      }
    ...
    if (mapBuf && mapBuf[0x00] === 0x72 && mapBuf[0x01] === 0x72) {
      return {
        ...
        SHA1: crypto
          .createHash("sha1")
          .update(
            Uint8Array.prototype.slice.call(mapBuf, 0, mapBuf.length - 20)
          )
          .digest("hex"),
        expectedSHA1: Buffer.from(
          Uint8Array.prototype.slice.call(mapBuf, mapBuf.length - 20)
        ).toString("hex"),
      };
    } else {
      return {};
  ...
  getGoToTarget(buf) {
    return [
      buf.readUInt16LE(OFFSETS.TARGET_X),
      buf.readUInt16LE(OFFSETS.TARGET_Y),
    ];
  }
