...
window.onload = function () {
  ...
  async function localCoordsToRobotCoords(imagePoint) {
    ...
    await new Promise((resolve) => {
      image.onload = function () {
        ...
        point.y = Math.round(
          (image.height / scaleFactor + topMap - imagePoint.y) * 50.0
        );
        resolve(point);
      };
    });
  ...
  function canvasXtoMapX(x) {
    return roundTwoDecimals(
      ((x - panOffsetX) / wheelZoom / zoomLevel + mapMinX) / scaleFactor
    );
  }
  function canvasYtoMapY(y) {
    return roundTwoDecimals(
      ((y - panOffsetY) / wheelZoom / zoomLevel + mapMinY) / scaleFactor
    );
  }
  ...
  function getOriginalY(transformedY) {
    return Math.floor(
      (map.IMAGE.dimensions.height / scaleFactor +
        map.IMAGE.position.top -
        transformedY) *
        scaleFactor -
        2 -
        mapMinY
  ...
  async function downEvent(e) {
    ...
    for (let i = 0; i < rects.length; i++) {
      ...
      const canvasMinRectX = Math.round(
        (rect.x + panOffsetX / wheelZoom) * wheelZoom
      );
      const canvasMinRectY = Math.round(
        (rect.y + panOffsetY / wheelZoom) * wheelZoom
      );

      const canvasMaxRectX = Math.round(
        (rect.x + rect.width + panOffsetX / wheelZoom) * wheelZoom
      );
      const canvasMaxRectY = Math.round(
        (rect.y + rect.height + panOffsetY / wheelZoom) * wheelZoom
      );
      ...
      const distanceRedDot = Math.sqrt(
        Math.pow(redDotOffsetX, 2) + Math.pow(redDotOffsetY, 2)
      );
      ...
      if (distanceRedDot < 10) {
        ...
        offsetRectX >= 0 &&
        offsetRectY >= 0 &&
        offsetRectX <= rect.width * wheelZoom + 1 &&
        offsetRectY <= rect.height * wheelZoom + 1
      ) {
    ...
    if (map?.OBSTACLES2) {
      map?.OBSTACLES2?.forEach((obstacle) => {
        const canvasX =
          getOriginalX(obstacle[0] / 50) * zoomLevel * wheelZoom + panOffsetX;
        const canvasY =
          getOriginalY(obstacle[1] / 50) * zoomLevel * wheelZoom + panOffsetY;
        ...
        if (distance <= 5) {
          ...
          socket.onmessage = function (event) {
            ...
            if (
              serverData.image &&
              applySafeImageSource(popupImage, serverData.image)
            ) {
              ...
              popupImage.addEventListener("click", function () {
                ...
                socket.onmessage = function (event) {
                  ...
                  if (
                    serverData.image &&
                    applySafeImageSource(largePhotoImage, serverData.image)
                  ) {
  ...
  async function moveEvent(e) {
    ...
    const mouseY = mouseYtoCanvasY(e);
    if (isDragging) {
      rects[selectedRect].x = roundTwoDecimals(
        (mouseX - panOffsetX - offsetRectX) / wheelZoom
      );
      rects[selectedRect].y = roundTwoDecimals(
        (mouseY - panOffsetY - offsetRectY) / wheelZoom
      );
    } else if (isResizing) {
      rects[selectedRect].width = roundTwoDecimals(
        (mouseX - panOffsetX - rects[selectedRect].x - redDotOffsetX) /
          wheelZoom
      );
      rects[selectedRect].height = roundTwoDecimals(
        (mouseY - panOffsetY - rects[selectedRect].y - redDotOffsetY) /
          wheelZoom
      );
    } else if (goToTarget) {
  ...
  canvas.addEventListener("wheel", (e) => {
    ...
    wheelZoom =
      roundTwoDecimals(Math.min(Math.max(wheelZoom, minZoom), maxZoom) * 100) /
      100;

    panOffsetX = roundTwoDecimals(
      clientX + panOffsetX - (clientX * wheelZoom) / prevZoom
    );
    panOffsetY = roundTwoDecimals(
      clientY + panOffsetY - (clientY * wheelZoom) / prevZoom
    );
  ...
  async function drawBackgroundImage() {
    ...
    image.onload = await function () {
      ...
      if (contentAspectRatio > aspectRatio) {
        console.log(
          "Aspect ratio is greater than canvas aspect ratio mapSizeX: " +
            mapSizeX +
            " mapSizeY: " +
            mapSizeY
        );
        ...
        console.log(
          "Aspect ratio is less than canvas aspect ratio mapSizeX: " +
            mapSizeX +
            " mapSizeY: " +
            mapSizeY
        );
  ...
  function drawMap(manualDraw = false) {
    if (manualDraw || isDragging || isResizing || isPanning || goToTarget) {
      ...
      for (let i = 0; i < rects.length; i++) {
        ...
        ctx.beginPath();
        ctx.arc(
          (rect.x + rect.width) / zoomLevel,
          (rect.y + rect.height) / zoomLevel,
          10,
          0,
          2 * Math.PI
        );
