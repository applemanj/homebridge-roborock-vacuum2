...
- **Matter Vacuum**: On Homebridge 2 with Matter enabled for the Roborock child bridge/daughter bridge, optionally expose each robot as a Matter robotic vacuum while keeping the existing HomeKit fan/switch accessory for compatibility. Matter Service Area room selection is included for clients that support area selection.
...
| `debugMode`                     | No             | Advanced troubleshooting option. Enables verbose API, discovery, and transport logging in the Homebridge log.                                                                                                                                                                                          |
...
| `enableMatter`                  | No             | Exposes each vacuum as a Matter robotic vacuum with Service Area room selection when running Homebridge 2 with Matter enabled for the Roborock child bridge/daughter bridge. After restart, pair using the bridge QR code and vacuum setup code shown in the plugin setup screen. Defaults to `false`. |
| `cloudOnlyMode`                 | No             | Advanced troubleshooting option. Disables local LAN discovery and local TCP commands for this plugin, routing commands and status polling through Roborock cloud when available. Defaults to `false`; restart after changing it.                                                                       |
| `preferCloudForMatterCommands`  | No             | Advanced troubleshooting option. Sends Matter vacuum commands through the Roborock cloud when cloud transport is connected. Useful for models whose local LAN command acknowledgements time out. Defaults to `false`.                                                                                  |
...
Homebridge 2 can expose supported accessories over Matter. If you enable **Enable Matter vacuum** in this plugin and Matter is enabled for your Roborock child bridge/daughter bridge, the plugin registers a second, Matter-native robotic vacuum accessory for each supported Roborock.
...
2. Enable **Enable Matter vacuum** in this plugin's settings.
3. Restart the Roborock child bridge.
4. Reopen the plugin setup screen and use the **Matter Pairing** section.
5. Add the Roborock child/daughter bridge in Apple Home using the bridge QR code.
6. If Apple Home asks to add the external Roborock vacuum accessory, use the vacuum's 11-digit setup code shown in the same section.

Treat the Matter QR code and manual/setup codes like temporary setup secrets; avoid posting screenshots or logs that include them publicly. If the setup screen cannot read the Homebridge Matter files, the Homebridge log still contains the same commissioning block as a fallback.
...
If a Matter vacuum stays on **Updating…** on a current Homebridge 2.1.x-beta, first remove the stale Matter vacuum from Apple Home, reset/restart the Roborock child bridge's Matter accessory, and pair it again. A live Homebridge 2.1.1-beta test initially reproduced the stuck tile with error-free Matter reads, but a clean reset/re-pair later rendered the full RVC endpoint correctly in Apple Home; the investigation is tracked in [homebridge/homebridge#3951](https://github.com/homebridge/homebridge/issues/3951). The HomeKit fan/switch accessory keeps working in the meantime.
